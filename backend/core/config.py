...
class Settings:
    ...
    auth_signup_per_minute: int = field(default_factory=lambda: int(os.getenv("AUTH_SIGNUP_PER_MINUTE", "10")))
    auth_login_per_minute: int = field(default_factory=lambda: int(os.getenv("AUTH_LOGIN_PER_MINUTE", "30")))
    rp_requirement_create_per_minute: int = field(default_factory=lambda: int(os.getenv("RP_REQUIREMENT_CREATE_PER_MINUTE", "10")))
    rp_source_request_per_minute: int = field(default_factory=lambda: int(os.getenv("RP_SOURCE_REQUEST_PER_MINUTE", "20")))
    rp_invite_per_minute: int = field(default_factory=lambda: int(os.getenv("RP_INVITE_PER_MINUTE", "30")))
    rp_candidate_submit_per_minute: int = field(default_factory=lambda: int(os.getenv("RP_CANDIDATE_SUBMIT_PER_MINUTE", "20")))
    rp_search_per_minute: int = field(default_factory=lambda: int(os.getenv("RP_SEARCH_PER_MINUTE", "60")))
    rp_daily_source_request_limit: int = field(default_factory=lambda: int(os.getenv("RP_DAILY_SOURCE_REQUEST_LIMIT", "50")))
    rp_daily_invite_limit: int = field(default_factory=lambda: int(os.getenv("RP_DAILY_INVITE_LIMIT", "100")))
    rp_daily_requirement_limit: int = field(default_factory=lambda: int(os.getenv("RP_DAILY_REQUIREMENT_LIMIT", "25")))
    rp_daily_candidate_limit: int = field(default_factory=lambda: int(os.getenv("RP_DAILY_CANDIDATE_LIMIT", "100")))
